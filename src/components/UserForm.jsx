...
function UserForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Добавляем username только если форма регистрации
    ...
    try {
      if (isLogin) {
        ...
        setUsername(response.split("Welcome, ")[1]?.split("!")[0]); // Извлекаем имя пользователя
  ...
  if (isAuthenticated) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gray-900 text-white">
        ...
        <button onClick={() => setIsAuthenticated(false)} className="p-2 bg-red-600 rounded-lg">
          Log Out
        </button>
      </div>
    );
